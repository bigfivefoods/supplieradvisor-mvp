-- Brief 37 — Every Advisor padded member income 4400-0000001 (not 4401/4500).
-- Recodes integer Member — 4401–4699 onto 4400-{lpad(customer.id,7)}.
-- Does not touch 1180-* AR, leftover 1190, or profile 102.
-- Paste after Brief 37 is on main. Do not re-run Brief 33 SQL.
-- Idempotent if Brief 36 already recoded 110 (padded rows stay).
-- Recodes any profile passed in; live call is 110 only. Do not run on 102.

CREATE OR REPLACE FUNCTION public.sa_brief37_recode_member_rev(p_profile_id integer)
...
SELECT public.sa_brief37_recode_member_rev(110);
-- Do not run on 102.
