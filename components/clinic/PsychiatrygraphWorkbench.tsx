...
import type { PsychiatrygraphStore } from '@/lib/clinic/psychiatrygraph';
import { RelationshipHeader } from '@/components/relationship/RelationshipChrome';

export function usePsychiatrygraph() {
  const companyId = getSelectedCompanyId()!;
  const [store, setStore] = useState<PsychiatrygraphStore | null>(null);
...
export function PsychiatrygraphWorkbench({
  title,
  titleAccent,
  description,
  children,
      ...
      <PsychiatrygraphPage>
        <div className="mb-4">
          <Link
            ...
            className="inline-flex items-center gap-1 text-xs font-bold text-indigo-700 hover:text-indigo-900 dark:text-indigo-300 dark:hover:text-white"
          >
            <ArrowLeft className="w-3.5 h-3.5" /> PsychiatryAdvisor
          </Link>
        </div>
        <RelationshipHeader
