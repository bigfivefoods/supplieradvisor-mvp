...
import type { VetgraphStore } from '@/lib/clinic/vetgraph';
import { RelationshipHeader } from '@/components/relationship/RelationshipChrome';

export function useVetgraph() {
  const companyId = getSelectedCompanyId()!;
  const [store, setStore] = useState<VetgraphStore | null>(null);
  ...
  const post = async (
    ...
    try {
      ...
      rememberAdvisorDeskCache('vetgraph', companyId, data);
      setStore(data.store);
      setSummary(data.summary || null);
      if (data.analysis) setAnalysis(data.analysis);
      return data;
...
export function VetgraphWorkbench({
  title,
  titleAccent,
  description,
  children,
