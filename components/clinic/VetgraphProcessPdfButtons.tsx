...
import { vetgraphProcessGuidePdfUrl } from '@/lib/clinic/vetgraph-process-guide-links';

type Variant = 'header' | 'inline' | 'map';

export default function VetgraphProcessPdfButtons({
  variant = 'inline',
  className = '',
}: {
  variant?: Variant;
  ...
  if (variant === 'header') {
    return (
      <div className={`flex flex-wrap items-center gap-2 ${className}`}>
        <a
          href={vetgraphProcessGuidePdfUrl('landscape', { download: true })}
        ...
        <a
          href={vetgraphProcessGuidePdfUrl('portrait', { download: true })}
  ...
  return (
    <div className={`flex flex-wrap items-center gap-2 ${className}`}>
      <a
        href={vetgraphProcessGuidePdfUrl('landscape')}
        target="_blank"
      ...
      <a
        href={vetgraphProcessGuidePdfUrl('portrait')}
        target="_blank"
        rel="noopener noreferrer"
        className="btn-secondary !py-1.5 !px-3 text-xs inline-flex items-center gap-1.5"
