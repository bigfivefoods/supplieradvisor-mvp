...
import PsychiatrygraphProcessPdfButtons from '@/components/clinic/PsychiatrygraphProcessPdfButtons';
...
export default function PsychiatrygraphSystemFlow({
  compact,
  ...
  return (
    <section
      ...
      aria-label="PsychiatryAdvisor full process design"
      id="psychiatrygraph-system-flow"
    >
      <div className="bg-gradient-to-r from-indigo-950 via-indigo-800 to-cyan-700 px-5 py-4 text-white">
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
          ...
          <div className="flex flex-wrap items-center gap-2 shrink-0">
            <PsychiatrygraphProcessPdfButtons variant="map" />
            <button
              type="button"
              onClick={() => setOpen((v) => !v)}
        ...
        <div className="p-4 sm:p-6 space-y-8">
          ...
          <div className="rounded-2xl border border-indigo-100 bg-indigo-50/50 p-4">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-2">
              {GUARDRAILS.map((g) => (
                <div
                  key={g.title}
                  className="rounded-xl bg-white border border-indigo-100 px-3 py-2.5"
                >
          ...
          <div className="rounded-2xl border border-indigo-100 bg-indigo-50/60 px-4 py-3">
            <div className="flex flex-col lg:flex-row lg:items-center gap-3">
              <div className="flex items-start gap-2 text-indigo-950 min-w-0 flex-1">
                <Sparkles className="w-5 h-5 shrink-0 text-indigo-700 mt-0.5" />
              ...
              <div className="flex flex-wrap gap-2 shrink-0">
                <Link
                  href="/dashboard/psychiatrygraph/calendar"
                  className="text-[11px] font-bold rounded-full bg-indigo-800 text-white px-3 py-1.5"
                >
                  Diary
                </Link>
                <Link
                  href="/dashboard/psychiatrygraph/website"
                  className="text-[11px] font-bold rounded-full bg-white border border-indigo-200 px-3 py-1.5 text-indigo-800"
                >
                  Website
                </Link>
                <Link
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 pt-1 border-t border-slate-100">
            ...
            <PsychiatrygraphProcessPdfButtons variant="inline" />
          </div>
        </div>
      ) : null}
    </section>
...
const ROLE_STYLES = {
  owner: {
    card:
      'border-indigo-300 bg-indigo-50/50 dark:border-indigo-400 dark:bg-indigo-950 dark:ring-1 dark:ring-indigo-500/40',
    badge: 'bg-indigo-700 dark:bg-indigo-500',
    chip: 'bg-indigo-700 text-white dark:bg-indigo-500 dark:text-white',
    title: 'text-slate-900 dark:text-indigo-100',
    subtitle: 'text-slate-500 dark:text-indigo-300/80',
    doesLabel: 'text-indigo-700 dark:text-indigo-300',
    doesText: 'text-slate-700 dark:text-indigo-50/90',
...
function PhaseStepCard({ step }: { step: PhaseStep }) {
  ...
  return (
    <Link
      href={step.href}
      className="flex-1 min-w-0 rounded-2xl border border-slate-200 bg-white px-3 py-3 hover:border-indigo-300 hover:bg-indigo-50/30 transition-colors dark:border-neutral-800 dark:bg-neutral-950 dark:hover:border-indigo-500/40 dark:hover:bg-neutral-900"
    >
