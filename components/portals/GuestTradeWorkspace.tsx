...
export function GuestTradeWorkspace({
  ...
  return (
    <div className="space-y-4">
      {ot && (ot.totalPOs > 0 || (ot.overall || 0) > 0) ? (
      ...
      {tab === 'newpo' && !isSupplier ? (
        <NewPoPanel busy={busy} onAct={act} />
      ) : null}
      {tab === 'riad' ? (
  ...
  return (
    <div className="rounded-[1.5rem] border border-white/70 bg-white/90 p-5 space-y-3">
      <p className="text-sm text-neutral-600">
        Raise a purchase order onto our books. Attach your PO file as a link if you
        have one.
      </p>
      <input
