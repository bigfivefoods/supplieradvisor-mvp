...
export default function LandingNav() {
  ...
  const scrollTo = (id: string) => {
    setOpen(false);
    // Wait for drawer unmount + body scroll unlock before scrolling
    window.setTimeout(() => {
  ...
  return (
    <>
      ...
      {/*
        CRITICAL: Only mount the mobile overlay when open.
        globals.css forces `pointer-events: auto !important` on buttons, so a
        closed full-screen overlay button would steal all mobile taps.
      */}
      {open && (
        <div className="fixed inset-0 z-[190] md:hidden" role="dialog" aria-modal="true">
          {/* Backdrop — separate from nav panel so it never blocks header (z-200) */}
          <div
