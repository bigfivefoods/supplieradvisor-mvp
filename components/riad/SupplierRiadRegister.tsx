...
import {
  ...
  priorityClass,
  statusClass,
  type SupplierRiadRecord,
  type RiadType,
} from '@/lib/suppliers/riad';
import type { SrmSupplierRecord } from '@/lib/suppliers/types';
import { RiadMetricsBoard } from '@/components/riad/RiadMetricsBoard';

type Props = {
  companyId: number;
  suppliers?: SrmSupplierRecord[];
  /** Optional fixed supplier (e.g. from a profile deep-link) */
  fixedSupplierId?: number | null;
  compact?: boolean;
};
...
export default function SupplierRiadRegister({
  companyId,
  suppliers = [],
  ...
  const [items, setItems] = useState<SupplierRiadRecord[]>([]);
  ...
  const [detail, setDetail] = useState<SupplierRiadRecord | null>(null);
  const [q, setQ] = useState('');
  const [statusFilter, setStatusFilter] = useState('open');
  const [supplierFilter, setSupplierFilter] = useState(
    fixedSupplierId ? String(fixedSupplierId) : 'all'
  );
  const [form, setForm] = useState({
    ...emptyForm,
    supplier_id: fixedSupplierId ? String(fixedSupplierId) : '',
  });
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const cid = fixedSupplierId || (supplierFilter !== 'all' ? Number(supplierFilter) : null);
      if (cid) params.set('supplierId', String(cid));
      if (tab !== 'all') params.set('type', tab);
      if (statusFilter !== 'all') params.set('status', statusFilter);

      const res = await fetch(`/api/suppliers/riad?${params}`);
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || 'Failed to load');
      const list = (data.items || data.entries || []) as SupplierRiadRecord[];
      setItems(
        list.map((e) => ({
          ...e,
  ...
  }, [companyId, fixedSupplierId, supplierFilter, tab, statusFilter]);
  ...
  const submit = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/suppliers/riad', {
        method: 'POST',
    ...
    try {
      ...
      if (next) {
        setDetail({
          ...
          supplier_name: next.supplier_name || item.supplier_name,
        });
      }
      setResolutionText('');
      void load();
  ...
  const closeItem = async (item: SupplierRiadRecord) => {
    const note =
      resolutionText.trim() ||
  ...
  const remove = async (item: SupplierRiadRecord) => {
    if (!confirm('Delete this RIAD entry?')) return;
    const res = await fetch(`/api/suppliers/riad?id=${item.id}&companyId=${companyId}`, { method: 'DELETE' });
    if (!res.ok) {
  ...
  return (
    <div className={compact ? '' : ''}>
      ...
      <p className="text-[11px] font-black uppercase tracking-[0.14em] text-[#0077b6] mb-3">
        Register
      </p>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-2">
        ...
        <div className="flex gap-2 items-center flex-wrap">
          ...
          {!fixedSupplierId && suppliers.length > 0 && (
            <select
        ...
        {loading ? (
          ...
          <div className="p-16 text-center">
            ...
            <p className="text-sm text-neutral-500 mb-4">
              Log risks, issues, actions, and decisions for supplier relationships.
            </p>
            <button
              type="button"
              onClick={() => openCreate()}
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-neutral-500 mt-0.5 flex flex-wrap gap-x-2">
                        <span>{item.supplier_name || 'General (no supplier)'}</span>
              ...
              <div className="overflow-y-auto p-5 space-y-4 flex-1 min-h-0">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="What happened / what needs attention for this supplier relationship"
                  />
                </div>
                {!fixedSupplierId && (
                  <div>
                    ...
                    >
                      <option value="">Company-wide (no specific supplier)</option>
                      {suppliers.map((c) => (
                        <option key={c.id} value={c.id}>
                          {c.trading_name}
                        </option>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    >
                      ...
                      {SUPPLIER_RIAD_CATEGORIES.map((c) => (
                ...
                {form.entry_type === 'risk' && (
                  <div>
                    ...
                    <textarea
                      ...
                      placeholder="How will this supplier risk be reduced or controlled?"
                    />
                  </div>
                )}
            ...
            <dl className="space-y-2 text-sm mb-6">
              <div className="flex justify-between gap-2">
                ...
                <dd className="font-medium text-right">
                  {detail.supplier_name || 'General (no supplier)'}
                </dd>
              </div>
              {detail.category && (
                <div className="flex justify-between gap-2">
              ...
              {detail.related_po_id != null && (
                <div className="flex justify-between gap-2">
                  <dt className="text-neutral-500">Related order</dt>
                  <dd className="font-medium">#{detail.related_po_id}</dd>
                </div>
              )}
              {detail.related_po_id != null && (
                <div className="flex justify-between gap-2">
                  <dt className="text-neutral-500">Related PO</dt>
                  <dd className="font-medium">#{detail.related_po_id}</dd>
                </div>
              )}
            </dl>
