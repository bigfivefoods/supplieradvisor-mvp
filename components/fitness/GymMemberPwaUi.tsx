...
export function GymSharePanel({
  ...
  const blurb = [
    `Train with ${brand} on GymAdvisor®`,
    bio ? String(bio).trim() : '',
    phone ? `Call ${phone}` : '',
    url,
  ]
  ...
  const shareNative = async () => {
    ...
    try {
      ...
      toast.success('Gym details copied');
    } catch {
  ...
  const copy = async () => {
    try {
      await navigator.clipboard.writeText(blurb);
      toast.success('Copied gym details');
    } catch {
  ...
  return (
    <div className="space-y-3">
      <GymSectionTitle hint="Send this gym to a friend on WhatsApp, socials, or copy the link.">
        Share {brand}
      </GymSectionTitle>
      <div
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        {email ? (
          <a
            href={`mailto:?subject=${encodeURIComponent(`Train at ${brand}`)}&body=${encodeURIComponent(blurb)}`}
            className="inline-flex min-h-11 items-center justify-center gap-2 rounded-2xl border border-slate-200 bg-white text-sm font-black text-slate-800 dark:border-white/10 dark:bg-neutral-900 dark:text-white"
          >
