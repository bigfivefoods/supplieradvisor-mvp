...
import { fitgraphProcessGuidePdfUrl } from '@/lib/fitness/fitgraph-process-guide-links';

type Variant = 'header' | 'inline' | 'map';

export default function FitgraphProcessPdfButtons({
  variant = 'inline',
  className = '',
}: {
  variant?: Variant;
  ...
  return (
    <div className={`flex flex-wrap items-center gap-2 ${className}`}>
      <a
        href={fitgraphProcessGuidePdfUrl('landscape')}
        target="_blank"
      ...
      <a
        href={fitgraphProcessGuidePdfUrl('portrait')}
        target="_blank"
        rel="noopener noreferrer"
        className="btn-secondary !py-1.5 !px-3 text-xs inline-flex items-center gap-1.5"
