/**
 * PsychiatryAdvisor® end-to-end process guide content + PDF.
 ...
 * Client UI should use `@/lib/clinic/psychiatrygraph-process-guide-links` only.
 */
...
import type { PsychiatrygraphProcessGuideOrientation } from '@/lib/clinic/psychiatrygraph-process-guide-links';
export type { PsychiatrygraphProcessGuideOrientation } from '@/lib/clinic/psychiatrygraph-process-guide-links';
    ...
    steps: [
      {
        ...
        title: 'Practitioners',
        who: 'Owner',
        desc: 'Psychiatrists, psychologists; disciplines, rates, bios. Permanent staff dual-write to People (HR).',
      },
      {
        n: '1b',
      ...
      },
      {
        ...
        who: 'Practitioner',
        desc: 'Clinical notes; medical aid, docs, claims; visit notes & outcome scores. Optional VerifyNow/Didit identity on portal.',
      },
    ],
  },
  {
    ...
    steps: [
      {
        ...
        title: 'Services',
        who: 'Owner',
        desc: 'Consults and sessions — duration and price.',
      },
      {
        n: '2b',
        title: 'Care packages',
    ...
    steps: [
      {
        ...
        who: 'Owner / desk',
        desc: 'Define rooms/rooms on Website; schedule date, time, service, room; assign clinician.',
      },
      {
        n: '3b',
...
export const SYSTEM_BENEFITS = [
  {
    title: 'Mental health OS',
    desc: 'Psychiatry and psychology on one diary and patient book.',
  },
  {
...
function geoFor(orientation: PsychiatrygraphProcessGuideOrientation): Geo {
...
function drawFooter(doc: PdfDoc, g: Geo, pageNum: number, total: number) {
  withOpenMargins(doc, () => {
    ...
    doc
      ...
      .fillColor(MUTED)
      .text(
        `SupplierAdvisor® · PsychiatryAdvisor® · ${orientLabel} · Clinic services OS`,
        g.mx,
        y + 4,
...
function drawHero(doc: PdfDoc, g: Geo): number {
  ...
  return drawProcessGuideHero(doc, g, {
    eyebrow: 'PsychiatryAdvisor® · end-to-end process · ' + orientLabel,
    title: 'Practitioners → Patients → Services → Diary → Bookings → Website',
    subtitle: g.isLandscape ? undefined : 'Tertiary / services mental health OS — people, diary, care packs, website & reports.',
    sideNote: g.isLandscape ? 'End-to-end mental health OS on SupplierAdvisor® — diary, packages, website booking.' : undefined,
...
function drawGuardrails(doc: PdfDoc, g: Geo, y: number): number {
  ...
  GUARDRAILS.forEach((item, i) => {
    ...
    doc.roundedRect(x, by, boxW, boxH, 4).fillAndStroke('#eef2ff', '#a5b4fc');
    doc
      .font('Helvetica-Bold')
      .fontSize(7)
      .fillColor(INK)
...
function drawOutcome(doc: PdfDoc, g: Geo, y: number): number {
  ...
  doc.roundedRect(g.mx, y, g.contentW, h, 6).fillAndStroke('#eef2ff', '#a5b4fc');
  doc
    .font('Helvetica-Bold')
    .fontSize(7)
    .fillColor(BRAND_DEEP)
...
export async function buildPsychiatrygraphProcessGuidePdf(opts?: {
  generatedAt?: Date;
  orientation?: PsychiatrygraphProcessGuideOrientation;
}): Promise<Buffer> {
  const generated = opts?.generatedAt || new Date();
  const orientation: PsychiatrygraphProcessGuideOrientation =
  ...
  return new Promise((resolve, reject) => {
    const doc = new PDFDocument({
      ...
      info: {
        Title:
          'PsychiatryAdvisor® Process Design — Practitioners → Diary → Website',
        Author: 'SupplierAdvisor®',
        Subject: `PsychiatryAdvisor clinic services end-to-end process (A4 ${orientation})`,
        Keywords:
          'PsychiatryAdvisor, clinic, physio, practitioners, diary, packages, process guide',
    ...
    y = drawProcessGuidePageHeader(doc, g, {
      eyebrow: 'PsychiatryAdvisor® · end-to-end process · continued',
...
export function parsePsychiatrygraphProcessGuideOrientation(
  raw: string | null | undefined
): PsychiatrygraphProcessGuideOrientation {
  const v = String(raw || '').toLowerCase();
  if (v === 'portrait' || v === 'p') return 'portrait';
  return 'landscape';
}

export function psychiatrygraphProcessGuideFilename(
  orientation: PsychiatrygraphProcessGuideOrientation
): string {
  return `PsychiatryAdvisor-Process-Design-A4-${orientation}.pdf`;
}
