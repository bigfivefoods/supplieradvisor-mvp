/**
 * VetAdvisor® — tertiary / services veterinary practice OS
 * (Vets, veterinary nurses, specialists: practitioners, patients, services,
 * care packages, diary, bookings, website).
 * Stored on profiles.metadata.vetgraph.
 */
...
export const VETGRAPH_MODULE_ID = 'vetgraph' as const;
export const VETGRAPH_META_KEY = 'vetgraph';
...
export type VetRateBasis = (typeof PRACTITIONER_RATE_BASES)[number] | string;

/** One closed employment / engagement period for a practitioner */
export type VetEngagement = {
  id: string;
  /** YYYY-MM-DD */
  start_date: string;
  /** YYYY-MM-DD */
...
export type VetgraphStore = {
  practitioners: VetPractitioner[];
  patients: VetPatient[];
  services: VetService[];
  packages: VetPackage[];
...
export function defaultPublicSettings(companyId?: number): VetPublicSettings {
  return {
    enabled: false,
    public_token:
      companyId != null
        ? `vetg_${companyId}_${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 10)}`
        : `vetg_${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 10)}`,
    ...
    embed_primary_color: '#c2410c',
    practitioner_disciplines: [...DEFAULT_PRACTITIONER_DISCIPLINES],
  };
}

export function emptyVetgraphStore(): VetgraphStore {
  return {
    practitioners: [],
    patients: [],
    services: [],
...
export function readVetgraphFromMetadata(
  meta: Record<string, unknown> | null | undefined
): VetgraphStore {
  if (!meta || typeof meta !== 'object') return emptyVetgraphStore();
  const raw = meta[VETGRAPH_META_KEY];
  if (!raw || typeof raw !== 'object') return emptyVetgraphStore();
  const s = raw as Partial<VetgraphStore>;
...
export const VETGRAPH_PATIENT_TOKENS_KEY = 'vetgraph_patient_tokens';
export const VETGRAPH_STAFF_TOKENS_KEY = 'vetgraph_staff_tokens';

export function writeVetgraphToMetadata(
  ...
  return {
    ...
    [VETGRAPH_PATIENT_TOKENS_KEY]: patientTokens,
    [VETGRAPH_STAFF_TOKENS_KEY]: staffTokens,
  };
}

/** Issue client portal token (self-serve diary booking). */
export function issuePatientPortalToken(companyId: number): string {
  return `vetp_${companyId}_${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 10)}`;
}

/** Issue practitioner diary portal token. */
export function issuePractitionerPortalToken(companyId: number): string {
  return `clin_${companyId}_vet_${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 10)}`;
}

export function parseVetCompanyIdFromToken(token: string): number | null {
  const m = /^vetp_(\d+)_/.exec(token);
  if (m) return Number(m[1]);
  const pg = /^vetg_(\d+)_/.exec(token);
...
/** @deprecated use parseVetCompanyIdFromToken */
export const parsePhysioCompanyIdFromToken = parseVetCompanyIdFromToken;

/**
 * Client portal: open public diary slots with vacancies + this patient's bookings.
 */
...
export function ensurePublicToken(
  settings: VetPublicSettings | undefined,
  companyId?: number
): VetPublicSettings {
  const base: VetPublicSettings = {
...
export function summariseVetgraph(store: VetgraphStore) {
...
export function seedDemoVetgraph(
  now: string,
  companyId?: number
): VetgraphStore {
