/**
 * constructiongraph API authz guard order.
 * Run: npx --yes tsx lib/construction/constructiongraph-authz.test.ts
 */
import assert from 'node:assert/strict';
import { readFileSync } from 'node:fs';
import { resolve } from 'node:path';

const routePath = resolve('app/api/construction/constructiongraph/route.ts');
const src = readFileSync(routePath, 'utf8');

function extractFn(name: 'GET' | 'POST'): string {
  const marker = `export async function ${name}(`;
  const start = src.indexOf(marker);
  assert.ok(start >= 0, `Could not find ${name} in constructiongraph route`);
  let depth = 0;
  let inside = false;
  let end = start;
