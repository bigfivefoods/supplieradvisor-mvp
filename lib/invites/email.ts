...
export function buildResellerInviteLink(token: string) {
  return `${getAppUrl()}/reseller/invite?token=${encodeURIComponent(token)}`;
}

/** Email body for container reseller portal invitation. */
export function resellerInviteEmailHtml(params: {
  resellerName?: string | null;
  companyName: string;
  containerName?: string | null;
  inviteLink: string;
}): string {
  const { resellerName, companyName, containerName, inviteLink } = params;
  const outlet = containerName
    ? ` linked to outlet <strong>${containerName}</strong>`
    : '';
  return `
<!DOCTYPE html>
<html>
<body style="margin:0;padding:0;background:#f8fafc;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;">
  <div style="max-width:620px;margin:32px auto;background:#fff;border-radius:20px;overflow:hidden;border:1px solid #e2e8f0;">
    <div style="background:linear-gradient(135deg,#00b4d8 0%,#0077b6 100%);padding:36px 40px;color:#fff;text-align:center;">
      <div style="font-size:13px;letter-spacing:0.12em;text-transform:uppercase;opacity:0.9;margin-bottom:8px;">SupplierAdvisor®</div>
      <h1 style="margin:0;font-size:26px;font-weight:800;letter-spacing:-0.5px;">Reseller portal invitation</h1>
    </div>
    <div style="padding:36px 40px;">
      <p style="color:#334155;font-size:16px;line-height:1.7;margin:0 0 18px;">
        Hello${resellerName ? ` ${resellerName}` : ''},
      </p>
      <p style="color:#334155;font-size:16px;line-height:1.7;margin:0 0 18px;">
        <strong>${companyName}</strong> has invited you as a <strong>container network reseller</strong>${outlet}.
      </p>
      <p style="color:#334155;font-size:16px;line-height:1.7;margin:0 0 12px;">
        In your portal you can:
      </p>
      <ul style="color:#475569;font-size:15px;line-height:1.6;margin:0 0 24px;padding-left:22px;">
        <li style="margin-bottom:6px;">See stock transferred to you from outlets</li>
        <li style="margin-bottom:6px;">Record sales and earn commission per item</li>
        <li style="margin-bottom:6px;">Track your earnings in one place</li>
      </ul>
      <p style="color:#334155;font-size:15px;line-height:1.7;margin:0 0 28px;">
        Click below to confirm and open your secure reseller portal. Sign in with
        <em>this email address</em> (one-time code, Google, or Apple).
      </p>
      <div style="text-align:center;margin:28px 0 32px;">
        <a href="${inviteLink}" style="background:#00b4d8;color:#fff;padding:16px 40px;border-radius:9999px;text-decoration:none;font-weight:700;font-size:16px;display:inline-block;">
          Confirm &amp; open reseller portal →
        </a>
      </div>
      <p style="color:#64748b;font-size:13px;line-height:1.6;margin:0;">
        If the button doesn't work, paste this link into your browser:<br/>
    ...
    <div style="background:#f8fafc;padding:20px 40px;border-top:1px solid #e2e8f0;font-size:12px;color:#64748b;text-align:center;">
      SupplierAdvisor® · Container reseller portal · Verified. Transparent.
    </div>
  </div>
</body>
</html>`;
...
/** Customer platform invite — claim via onboarding with kind=customer. */
...
<body style="margin:0;padding:0;background:#f8fafc;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;">
  <div style="max-width:620px;margin:32px auto;background:#fff;border-radius:20px;overflow:hidden;border:1px solid #e2e8f0;">
    <div style="background:linear-gradient(135deg,#00b4d8 0%,#0077b6 100%);padding:36px 40px;color:#fff;text-align:center;">
      <div style="font-size:13px;letter-spacing:0.12em;text-transform:uppercase;opacity:0.9;margin-bottom:8px;">SupplierAdvisor®</div>
      <h1 style="margin:0;font-size:26px;font-weight:800;letter-spacing:-0.5px;">You're invited as a customer</h1>
    </div>
    <div style="padding:36px 40px;">
      <p style="color:#334155;font-size:16px;line-height:1.7;margin:0 0 18px;">
        Hello${inviteeName ? ` ${inviteeName}` : ''},
      ...
      <p style="color:#334155;font-size:16px;line-height:1.7;margin:0 0 18px;">
        <strong>${invitedBy}</strong> at <strong>${sellerCompanyName}</strong> has invited
        <strong>${customerName}</strong> to join SupplierAdvisor as a connected customer —
        raise purchase orders, view shared documents, and collaborate securely.
      </p>
      ${personalNote}
      <p style="color:#334155;font-size:16px;line-height:1.7;margin:0 0 28px;">
        Click below to sign in (or create your account) with this email address and accept the invitation.
