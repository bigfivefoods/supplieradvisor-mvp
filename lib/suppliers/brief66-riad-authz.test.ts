/**
 * Brief 66 — supplier RIAD route authz regression test
 * Run: npx --yes tsx lib/suppliers/brief66-riad-authz.test.ts
 */
...
const routePath = resolve('app/api/suppliers/riad/route.ts');
const src = readFileSync(routePath, 'utf8');

function extractFn(name: string): string {
  const marker = `export async function ${name}(`;
  const start = src.indexOf(marker);
  assert.ok(start >= 0, `Could not find ${name} in riad/route.ts`);
  let depth = 0;
  let inside = false;
  let end = start;
...
function loadRoute(opts: LoadOptions = {}) {
  const store: Record<string, Row[]> = {
    supplier_riad: [...(opts.rows || [])],
    srm_suppliers: [{ id: 77, profile_id: opts.allowCompanyId ?? 110, trading_name: 'Acme Foods' }],
  };
  const events: string[] = [];
  ...
  return {
    handlers: runner(
      ...
      (s?: string | null) => ['closed', 'resolved', 'cancelled'].includes(String(s || '').toLowerCase()),
      (s?: string | null) => !['closed', 'resolved', 'cancelled'].includes(String(s || '').toLowerCase()),
    ) as {
...
async function main() {
  {
    ...
    const response = await env.handlers.PATCH(
      makeRequest('https://example.test/api/suppliers/riad', { id: 1, companyId: 110, status: 'open' })
    );
    assert.equal(response.status, 401, 'unauth PATCH should be denied');
    assert.equal(env.supabaseCalls, 0, 'unauth PATCH must not query riad');
...

  {
    const env = loadRoute({ denyStatus: 401 });
    const response = await env.handlers.PATCH(
      makeRequest('https://example.test/api/suppliers/riad', { id: 1, status: 'open' })
    );
...

  {
    const env = loadRoute({ denyStatus: 401 });
    const response = await env.handlers.PATCH(
      makeRequest('https://example.test/api/suppliers/riad', { id: -1, companyId: 110 })
    );
...

  {
    ...
    const response = await env.handlers.DELETE(
      makeRequest('https://example.test/api/suppliers/riad?id=1')
    );
...

  {
    ...
    const response = await env.handlers.GET(
      makeRequest('https://example.test/api/suppliers/riad?companyId=0')
    );
...

  {
    ...
    const getResponse = await env.handlers.GET(
      makeRequest('https://example.test/api/suppliers/riad?companyId=102')
    );
    const patchResponse = await env.handlers.PATCH(
      makeRequest('https://example.test/api/suppliers/riad', { id: 1, companyId: 102 })
    );
    const deleteResponse = await env.handlers.DELETE(
      makeRequest('https://example.test/api/suppliers/riad?id=1&companyId=102')
    );
    assert.equal(getResponse.status, 403, 'member of 110 must not read company 102 riad');
    assert.equal(patchResponse.status, 403, 'member of 110 must not update company 102 riad');
    assert.equal(deleteResponse.status, 403, 'member of 110 must not delete company 102 riad');
...

  {
    ...
    const blockedUpdate = await env.handlers.PATCH(
      makeRequest('https://example.test/api/suppliers/riad', { id: 2, companyId: 110, status: 'closed' })
    );
    assert.equal(blockedUpdate.status, 404, 'PATCH must not update cross-company riad rows');

    const okUpdate = await env.handlers.PATCH(
      makeRequest('https://example.test/api/suppliers/riad', { id: 1, companyId: 110, title: 'Updated' })
    );
    const updateBody = await readBody(okUpdate);
    assert.equal(okUpdate.status, 200);
    assert.equal(updateBody.success, true);
    assert.equal(env.store.supplier_riad.find((row) => row.id === 1)?.title, 'Updated');
    assert.equal(env.store.supplier_riad.find((row) => row.id === 2)?.title, 'Other');
  }

  {
    ...
    const response = await env.handlers.DELETE(
      makeRequest('https://example.test/api/suppliers/riad?id=2&companyId=110')
    );
    const body = await readBody(response);
    assert.equal(response.status, 200);
    assert.equal(body.success, true);
    assert.equal(env.store.supplier_riad.length, 2, 'DELETE must not remove row from other profile');
  }

  {
    ...
    assert.equal(response.status, 200);
    assert.equal(body.success, true);
    assert.equal(env.store.supplier_riad.length, 1, 'DELETE should remove matching row');
    assert.equal(env.store.supplier_riad[0].id, 2);
  }

  {
    const env = loadRoute({ denyStatus: 401 });
    const response = await env.handlers.GET(
      makeRequest('https://example.test/api/suppliers/riad?companyId=110')
    );
...

  {
    ...
    const response = await env.handlers.POST(
      makeRequest('https://example.test/api/suppliers/riad', { companyId: 110, title: 'Still gated' })
    );
  ...
  console.log('✓ Brief 66 supplier RIAD authz assertions passed');
}
