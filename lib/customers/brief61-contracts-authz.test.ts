/**
 * Brief 61 — contracts route authz regression test
 * Run: npx --yes tsx lib/customers/brief61-contracts-authz.test.ts
 */
...
const routePath = resolve('app/api/customers/contracts/route.ts');
const src = readFileSync(routePath, 'utf8');

function extractFn(name: string): string {
  const marker = `export async function ${name}(`;
  const start = src.indexOf(marker);
  assert.ok(start >= 0, `Could not find ${name} in contracts/route.ts`);
  let depth = 0;
  let inside = false;
  let end = start;
...
assert.match(patchFn, /!Number\.isFinite\(contractId\)\s*\|\|\s*contractId\s*<=\s*0/);
...
assert.match(flatPatch, /\.update\(updates\)\s*\.eq\('id', contractId\)\s*\.eq\('profile_id', companyId\)/);
...
function loadRoute(opts: LoadOptions = {}) {
  const store: Record<string, Row[]> = {
    customer_contracts: [...(opts.contracts || [])],
    customers: [{ id: 77, profile_id: opts.allowCompanyId ?? 110, linked_profile_id: 999 }],
  };
  const events: string[] = [];
...
async function main() {
  {
    ...
    const response = await env.handlers.PATCH(
      makeRequest('https://example.test/api/customers/contracts', { id: 1, companyId: 110, title: 'x' })
    );
    assert.equal(response.status, 401, 'unauth PATCH should be denied');
    assert.equal(env.supabaseCalls, 0, 'unauth PATCH must not query contracts');
...

  {
    const env = loadRoute({ denyStatus: 401 });
    const response = await env.handlers.PATCH(
      makeRequest('https://example.test/api/customers/contracts', { id: 1, title: 'x' })
    );
...

  {
    ...
    const response = await env.handlers.DELETE(
      makeRequest('https://example.test/api/customers/contracts?id=1')
    );
...

  {
    const env = loadRoute({ denyStatus: 401 });
    const response = await env.handlers.DELETE(
      makeRequest('https://example.test/api/customers/contracts?id=-1&companyId=110')
    );
...

  {
    ...
    const patchResponse = await env.handlers.PATCH(
      makeRequest('https://example.test/api/customers/contracts', { id: 1, companyId: 102, title: 'x' })
    );
    const deleteResponse = await env.handlers.DELETE(
      makeRequest('https://example.test/api/customers/contracts?id=1&companyId=102')
    );
    assert.equal(patchResponse.status, 403, 'member of 110 must not update company 102 contracts');
    assert.equal(deleteResponse.status, 403, 'member of 110 must not delete company 102 contracts');
...

  {
    ...
    const blockedUpdate = await env.handlers.PATCH(
      makeRequest('https://example.test/api/customers/contracts', { id: 2, companyId: 110, title: 'Nope' })
    );
    assert.equal(blockedUpdate.status, 404, 'PATCH must not update cross-company contract rows');

    const okUpdate = await env.handlers.PATCH(
      makeRequest('https://example.test/api/customers/contracts', { id: 1, companyId: 110, title: 'Updated' })
    );
    const updateBody = await readBody(okUpdate);
    assert.equal(okUpdate.status, 200);
    assert.equal(updateBody.success, true);
    assert.equal(env.store.customer_contracts.find((row) => row.id === 1)?.title, 'Updated');
    assert.equal(env.store.customer_contracts.find((row) => row.id === 2)?.title, 'Other');
  }

  {
    ...
    const response = await env.handlers.DELETE(
      makeRequest('https://example.test/api/customers/contracts?id=2&companyId=110')
    );
    const body = await readBody(response);
    assert.equal(response.status, 200);
    assert.equal(body.success, true);
    assert.equal(env.store.customer_contracts.length, 2, 'DELETE must not remove row from other profile');
  }

  {
    ...
    assert.equal(response.status, 200);
    assert.equal(body.success, true);
    assert.equal(env.store.customer_contracts.length, 1, 'DELETE should remove matching row');
    assert.equal(env.store.customer_contracts[0].id, 2);
  }

  {
    const env = loadRoute({ denyStatus: 401 });
    const response = await env.handlers.GET(
      makeRequest('https://example.test/api/customers/contracts?companyId=110')
    );
...

  {
    ...
    const response = await env.handlers.POST(
      makeRequest('https://example.test/api/customers/contracts', { companyId: 110, title: 'Still gated' })
    );
  ...
  console.log('✓ Brief 61 contracts authz assertions passed');
}
