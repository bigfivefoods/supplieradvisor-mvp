/**
 * Brief 63 — customer RIAD route authz regression test
 * Run: npx --yes tsx lib/customers/brief63-riad-authz.test.ts
 */
...
const routePath = resolve('app/api/customers/riad/route.ts');
...
// PATCH scopes with profile_id at least twice (pre-check + update + retry)
...
// Pre-select must appear before the update call
assert.ok(
...
function sanitizeFn(fn: string): string {
  return fn
    ...
    .replace(/\(([^()]+?)\s+as\s+[^)]+\)/g, '($1)')
    // Remove optional TypeScript param annotations like (s?: string | null)
    .replace(/(\w)\?:\s*[\w\s|<>[\]]+(?=[,)])/g, '$1');
}
...
async function main() {
  // PATCH — unauth denied before Supabase
  {
    ...
    const response = await env.handlers.PATCH(
      makeRequest('https://example.test/api/customers/riad', { id: 1, companyId: 110, status: 'open' })
    );
  ...
  // PATCH — missing companyId → 400
  {
    const env = loadRoute({ denyStatus: 401 });
    const response = await env.handlers.PATCH(
      makeRequest('https://example.test/api/customers/riad', { id: 1, status: 'open' })
    );
  ...
  // PATCH — non-positive id → 400
  {
    const env = loadRoute({ denyStatus: 401 });
    const response = await env.handlers.PATCH(
      makeRequest('https://example.test/api/customers/riad', { id: -1, companyId: 110 })
    );
  ...
  // DELETE — missing companyId → 400
  {
    ...
    const response = await env.handlers.DELETE(
      makeRequest('https://example.test/api/customers/riad?id=1')
    );
  ...
  // Wrong company → 403 before Supabase
  {
    ...
    const patchResponse = await env.handlers.PATCH(
      makeRequest('https://example.test/api/customers/riad', { id: 1, companyId: 102 })
    );
  ...
  // PATCH scopes by profile_id — cross-company row returns 404
  {
    ...
    const blockedUpdate = await env.handlers.PATCH(
      makeRequest('https://example.test/api/customers/riad', { id: 2, companyId: 110, status: 'closed' })
    );
    ...
    const okUpdate = await env.handlers.PATCH(
      makeRequest('https://example.test/api/customers/riad', { id: 1, companyId: 110, title: 'Updated' })
    );
    const updateBody = await readBody(okUpdate);
    assert.equal(okUpdate.status, 200);
    assert.equal(updateBody.success, true);
    assert.equal(env.store.customer_riad.find((row) => row.id === 1)?.title, 'Updated');
    assert.equal(env.store.customer_riad.find((row) => row.id === 2)?.title, 'Other');
  }
  ...
  // DELETE scopes by profile_id — cross-company row not deleted
  {
    ...
    const response = await env.handlers.DELETE(
      makeRequest('https://example.test/api/customers/riad?id=2&companyId=110')
    );
    const body = await readBody(response);
    assert.equal(response.status, 200);
    assert.equal(body.success, true);
    assert.equal(env.store.customer_riad.length, 2, 'DELETE must not remove row from other profile');
  }
  ...
  // DELETE removes own row
  {
    ...
    assert.equal(response.status, 200);
    assert.equal(body.success, true);
    assert.equal(env.store.customer_riad.length, 1, 'DELETE should remove matching row');
    assert.equal(env.store.customer_riad[0].id, 2);
  }
  ...
  // GET remains gated
  {
    ...
    const response = await env.handlers.GET(
      makeRequest('https://example.test/api/customers/riad?companyId=110')
    );
  ...
  // POST remains gated
  {
    ...
    const response = await env.handlers.POST(
      makeRequest('https://example.test/api/customers/riad', { companyId: 110, title: 'Still gated' })
    );
  ...
  console.log('✓ Brief 63 RIAD authz assertions passed');
}
