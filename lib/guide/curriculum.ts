...
export const GUIDE_SECTIONS: GuideSection[] = [
  ...
  },
  {
    ...
    principles: [
      {
        ...
        body: 'Every public class should have a coach; rooms optional; reassign on the calendar anytime.',
      },
      {
    ...
    outcomes: [
      ...
      'Plans + one active subscription (freeze understood)',
      'Sessions scheduled with coach, room optional, capacity',
    ...
    processes: [
      {
        ...
        summary: 'Class type + room + coach + public flag → website calendar.',
        steps: [
      ...
      {
        name: 'Subscriptions, waitlist & attendance',
      ...
      },
      {
        ...
        summary: 'List publicly; message members in-app by system ID.',
        steps: [
          'Website — ops policies: reschedule + list on /marketplace/advisors',
          'Messages — desk/coach → member (in-app when on-system)',
          'Confirm delivery status on the thread',
        ],
      },
    ],
    concepts: [
      ...
      { term: 'Subscription', meaning: 'Client plan entitlement with status, remaining credits, and freeze.' },
      { term: 'System user ID', meaning: 'Platform user id that delivers care messages in-app once the member is on SupplierAdvisor.' },
    ],
    checklist: [
      'Coach + class type + scheduled session (room optional)',
      'One subscription or demo seed loaded',
      'Website + marketplace options understood',
  ...
  },
  {
    ...
    principles: [
      {
        ...
        body: 'Each clinician diary is exclusive; practice view can run many clinicians in parallel.',
      },
      {
        title: 'Public = published',
    ...
    flow: [
      { id: 'a', label: 'People', hint: 'Practitioners · POPIA', tone: 'cyan' },
      { id: 'b', label: 'Services', hint: 'Packs · plans', tone: 'emerald' },
      { id: 'c', label: 'Diary', hint: 'Rooms · clinician', tone: 'cyan' },
    ...
    processes: [
      {
        ...
        summary: 'Rooms + clinician exclusive books + public slots.',
        steps: [
          'Website — define rooms / resources',
          'Services — catalogue with duration',
          'Calendar — practice vs clinician view; schedule; assign; mark public',
          'Confirm no double-book on the same clinician',
        ],
      },
      {
        ...
        steps: [
          'Patients — create with POPIA consent; invite portal',
          'Bookings — book patient or family; other clinician if preferred full',
          'Waitlist desk — slot queue + next-available practice queue',
          'Send 24h reminders; mark attended / no-show',
          'Treatment plan on patient record — Book next when due',
        ],
      },
      {
    ...
    concepts: [
      ...
      { term: 'Treatment plan', meaning: 'Step plan on the patient with one-click book of the next open diary slot.' },
      { term: 'System user ID', meaning: 'Platform user id that delivers care messages in-app once the patient is on SupplierAdvisor.' },
  ...
  },
  {
    ...
    flow: [
      { id: 'a', label: 'People', hint: 'Staff · POPIA', tone: 'cyan' },
      { id: 'b', label: 'Services', hint: 'Packs · plans', tone: 'emerald' },
      { id: 'c', label: 'Diary', hint: 'Rooms · clinician', tone: 'cyan' },
      { id: 'd', label: 'Floor', hint: 'Waitlist · recall', tone: 'rose' },
      { id: 'e', label: 'Messages', hint: 'System ID', tone: 'violet' },
  ...
  },
  {
    ...
    tagline: 'Public sector · DBE → school → SP → safe kitchen → learners',
    purpose:
      'SchoolAdvisor® — National School Nutrition Programme OS on the government / public-sector pathway only: department govern, school kitchen & learners (including CoA / R638 food safety), SP supply, claim gates and prizes. Expandable process design + printable PDF on SchoolAdvisor hubs.',
    who: ['DBE / PEU', 'School kitchen', 'Service provider'],
    flow: [
      { id: 'a', label: 'Govern', hint: 'DBE', tone: 'slate' },
      { id: 'b', label: 'School', hint: 'Kitchen · CoA', tone: 'emerald' },
      { id: 'c', label: 'SP', hint: 'Supply · OTIFEF', tone: 'amber' },
      { id: 'd', label: 'Serve', hint: 'Meals · micro-log', tone: 'cyan' },
      { id: 'e', label: 'Prove', hint: 'Match · claim · prize', tone: 'violet' },
    ],
    processes: [
      {
        ...
        steps: [
          ...
          'Confirm packaging is Public Sector (auto-applied on hub load)',
      ...
      {
        name: 'School kitchen golden path',
        href: '/dashboard/schools/serve-day',
        summary: 'Stock → PO → receive → CoA/R638 → serve → claim.',
        steps: [
          'Set kitchen CoA / R638 passport (PIC + monthly self-audit)',
          'Check kitchen stock vs menu (Kitchen)',
          'Raise PO to linked SP when short',
          'Confirm POD / GRN on deliveries',
          'Log serve day + R638 micro-log (desk or field PWA)',
          'Submit claim only when match + SP SLA + kitchen safety are green',
        ],
      },
      {
        name: 'Kitchen food safety (R638)',
        href: '/dashboard/schools/kitchen-safety',
        summary: 'Legal kitchen passport — CoA, PIC, self-audit, PEU verify, claim gates.',
        steps: [
          'Record Certificate of Acceptability status and expiry',
          'Name Person in Charge and hygiene training date',
          'Complete monthly R638 self-audit checklist',
          'Log daily safety checks on serve day',
          'PEU verifies kitchen on field visits',
          'DBE tracks Valid CoA % and may set hard claim gate',
        ],
      },
      {
        name: 'DBE exception cockpit',
        href: '/dashboard/schools/ops',
        summary: 'Daily board for joins, claims, kitchen CoA risk, late deliveries, SP probation.',
        steps: [
          'Open hub → exception board',
          'Approve pending school joins',
          'Review kitchen CoA missing / expired / red kitchens',
          'Review submitted claims',
          'Chase late deliveries / off-catalogue GRNs',
          'Review SP OTIFEF / SLA',
        ],
      },
      {
        name: 'Field PWAs',
        href: '/dashboard/schools',
        summary: 'Kitchen serve day and PEU visit without desk login.',
        steps: [
          'School hub → copy serve day field link',
          'Add to kitchen phone home screen',
          'PEU: copy PEU visit link for monitors',
          'Record kitchen CoA outcome on PEU visit',
    ...
    checklist: [
      ...
      'Opened process design once',
      'Kitchen CoA / R638 passport started',
      'First serve day + micro-log logged',
      'Field serve link issued (schools)',
      'Exception cockpit checked (DBE)',
    ],
