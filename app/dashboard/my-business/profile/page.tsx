...
function ProfileContent() {
  ...
  // Handle general certificate upload
  ...
  const saveProfile = async () => {
    ...
    const { error } = await supabase.from('profiles').update({
      ...
      uploaded_certificates: uploadedCertificates,
    }).eq('id', Number(companyId));
  ...
  if (loading) return <div className="p-12 text-center">Loading company profile...</div>;
  ...
  return (
    <div className="px-4 md:px-8 lg:pr-12 py-8 lg:py-12 max-w-screen-2xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-10">
        <div>
          <h1 className="font-black text-4xl md:text-5xl tracking-[-2px]">{form.legal_name}</h1>
          <p className="text-xl text-neutral-600 mt-1">Profile &amp; Legal Information</p>
        </div>
        <div className={`px-5 py-2 rounded-3xl text-sm font-semibold self-start md:self-auto ${badgeColor}`}>
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-3xl border border-neutral-200 overflow-hidden">
          ...
          {expanded.basics && (
            <div className="px-8 pb-8 pt-2 grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div><label className="text-sm font-medium text-neutral-600">Contact Name</label>
                <input className="input w-full mt-1" value={form.contact_name || ''} onChange={e => handleInputChange('contact_name', e.target.value)} /></div>
              <div><label className="text-sm font-medium text-neutral-600">Contact Number</label>
              ...
              <div><label className="text-sm font-medium text-neutral-600">Email Address</label>
                <input className="input w-full mt-1" value={form.email || ''} onChange={e => handleInputChange('email', e.target.value)} /></div>
              <div><label className="text-sm font-medium text-neutral-600">Registration Number (CIPC)</label>
        ...
        <div className="bg-white rounded-3xl border border-neutral-200 overflow-hidden">
          ...
          {expanded.location && (
            <div className="px-8 pb-8 pt-2 grid grid-cols-1 md:grid-cols-2 gap-6">
              <div><label className="text-sm font-medium text-neutral-600">Continent</label>
              ...
              <div><label className="text-sm font-medium text-neutral-600">Country</label>
              ...
              <div><label className="text-sm font-medium text-neutral-600">Province / State</label>
              ...
              <div><label className="text-sm font-medium text-neutral-600">City</label><input className="input w-full mt-1" value={form.city || ''} onChange={e => handleInputChange('city', e.target.value)} /></div>
              <div><label className="text-sm font-medium text-neutral-600">Street Address</label><input className="input w-full mt-1" value={form.street || ''} onChange={e => handleInputChange('street', e.target.value)} /></div>
              <div><label className="text-sm font-medium text-neutral-600">Postal Code</label><input className="input w-full mt-1" value={form.postal_code || ''} onChange={e => handleInputChange('postal_code', e.target.value)} /></div>
            </div>
          )}
        </div>
          ...
          {expanded.financial && (
            <div className="px-8 pb-8 pt-2 space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div><label className="text-sm font-medium text-neutral-600">IBAN</label>
                  <input className="input w-full mt-1" value={form.iban || ''} onChange={e => handleInputChange('iban', e.target.value)} /></div>
                <div><label className="text-sm font-medium text-neutral-600">SWIFT / BIC</label>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t">
                <div>
                  <label className="text-sm font-medium text-neutral-600">Banking Confirmation</label>
                ...
                <div>
                  <label className="text-sm font-medium text-neutral-600">Export License Number</label>
                ...
                <div>
                  <label className="text-sm font-medium text-neutral-600">Import License Number</label>
            ...
            <div className="px-8 pb-8 pt-2 space-y-10">
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    <label className="text-sm font-medium text-neutral-600">BEE Level</label>
                  ...
                  <div className="md:col-span-2">
                    <label className="text-sm font-medium text-neutral-600">BEE Certificate</label>
              ...
              <div>
                <div className="flex items-center justify-between mb-4">
                  ...
                  <label className="cursor-pointer inline-flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 text-sm">
                    ...
                    Upload Certificate
                    <input type="file" className="hidden" accept=".pdf,.jpg,.jpeg,.png" onChange={handleGeneralCertificateUpload} />
                  </label>
                </div>
                      ...
                      <div key={index} className="flex items-center justify-between p-4 border border-gray-200 rounded-lg hover:bg-gray-50">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-gray-500">Uploaded {new Date(cert.uploadedAt).toLocaleDateString()}</p>
                          </div>
                        </div>
                        <a href={cert.url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-blue-600 hover:text-blue-700 hover:bg-blue-50 rounded-lg transition-colors">
        ...
        <div className="bg-white rounded-3xl border border-neutral-200 overflow-hidden">
          ...
          {expanded.verification && (
            <div className="px-8 pb-8 pt-2">
              <label className="text-sm font-medium text-neutral-600">Director ID Number (for VerifyNow)</label>
      ...
      <div className="flex flex-col sm:flex-row justify-end gap-4 mt-10">
        <button onClick={saveProfile} disabled={saving} className="btn-primary px-10 py-4 disabled:opacity-70">
          {saving ? 'Saving...' : 'Save All Changes'}
        </button>
        <button onClick={handleGetVerified} disabled={verifying} className="bg-emerald-600 hover:bg-emerald-700 text-white px-10 py-4 rounded-2xl font-semibold disabled:opacity-70">
          {verifying ? 'Processing...' : 'Get Verified - R69'}
        </button>
      </div>
    </div>
  );
