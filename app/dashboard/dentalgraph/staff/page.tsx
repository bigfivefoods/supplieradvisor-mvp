...
function formatEngagement(p: DentalStaff) {
  const start =
...
async function copyClinicianPortal(tok: string) {
  ...
  const url = `${window.location.origin}/clinician/dentalgraph/${encodeURIComponent(tok)}`;
...
async function copyStaffToday(tok: string) {
  if (typeof window === 'undefined') return;
  const url = `${window.location.origin}/staff/advisor/dentalgraph/${encodeURIComponent(tok)}`;
  await navigator.clipboard.writeText(url);
...
function profileFromPerson(p: DentalStaff): ProfileDraft {
  return {
    code: p.code || '',
    name: p.name || '',
    email: p.email || '',
...
export default function StaffPage() {
  ...
  const skillOptions = useMemo(
    () => (store ? getStaffRoleOptions(store) : []),
    [store]
  );
  ...
  const toggleSkill = (s: string) => {
    setForm((f) => ({
      ...f,
      roles: toggleInList(f.roles, s),
    }));
  };

  const profileFor = (p: DentalStaff): ProfileDraft =>
    profileDrafts[p.id] || profileFromPerson(p);

  const setProfile = (id: string, patch: Partial<ProfileDraft>) => {
    setProfileDrafts((prev) => {
      const person = store?.staff.find((x) => x.id === id);
      const base =
        prev[id] ||
        (person
  ...
  const draftFor = (p: DentalStaff): DateDraft => {
  ...
  const add = async () => {
    ...
    await post({
      ...
      record: {
        ...
        roles: form.roles.length ? form.roles : ['General'],
        can_manage: true,
  ...
  const saveDates = async (p: DentalStaff) => {
    const d = draftFor(p);
    if (!d.start_date) {
  ...
  const endTenure = async (p: DentalStaff) => {
    const d = draftFor(p);
  ...
  const rehire = async (p: DentalStaff) => {
    const d = draftFor(p);
    const start =
    ...
    toast.success('Staff rehired — new engagement started, history kept');
  };

  const saveIdentity = async (p: DentalStaff) => {
  ...
  const saveContracts = async (p: DentalStaff, contracts: FitContractDoc[]) => {
    await post({
      entity: 'staff',
      action: 'upsert',
      record: {
        ...
        code: p.code,
        name: p.name,
        contracts: contracts as DentalContractDoc[],
      },
    });
  };

  const saveDetails = async (p: DentalStaff) => {
    const pr = profileFor(p);
    if (!pr.name.trim()) {
      toast.error('Name required');
      return;
  ...
  const addSkill = async () => {
    ...
    if (!name) {
      toast.error('Enter a role / skill name');
      return;
    }
    await post({
      action: 'manage_roles',
  ...
  const saveRenameSkill = async () => {
    ...
    toast.success(`Renamed to “${to}” (updated on staff too)`);
  ...
  const removeSkill = async (name: string) => {
    if (
      !confirm(
        `Remove “${name}” from the roles list? Staff keep it on their profile unless you strip it.`
      )
    ) {
      return;
    }
    const strip = confirm(
      'Also remove this role from all staff who have it selected?'
    );
    await post({
      action: 'manage_roles',
      op: 'remove',
      name,
      strip_from_staff: strip,
    });
    toast.success(`Role “${name}” removed from catalogue`);
        ...
        <div className="space-y-6">
          <StatRow
            items={[
              {
                label: 'Staff',
                value: Number(summary?.staffCount) || store.staff.length,
              },
          ...
          <div className="space-y-2">
            {(() => {
              const incomplete = store.staff.filter(needsAdvisorIdentity).length;
              return incomplete > 0 ? (
                <div className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-[12px] dark:border-amber-700/50 dark:bg-amber-950/40">
                  <p className="font-semibold text-amber-950 dark:text-amber-100">
                    {incomplete} team member{incomplete === 1 ? '' : 's'} still
            ...
            {(onlyIncomplete
              ? store.staff.filter(needsAdvisorIdentity)
              : store.staff
            ).map((p) => {
                  ...
                  actions={
                    ...
                    <>
                      <button
                        type="button"
                        disabled={saving}
                        className="inline-flex items-center gap-1 text-xs font-bold text-sky-900 border border-sky-300 rounded-xl px-2.5 py-1.5 bg-sky-50 hover:bg-sky-100 dark:text-sky-100 dark:border-sky-500/50 dark:bg-sky-950 dark:hover:bg-sky-900/50"
                        onClick={() =>
                      ...
                      <button
                        type="button"
                        disabled={saving}
                        className="inline-flex items-center gap-1 text-xs font-bold text-slate-700 border border-slate-200 rounded-xl px-2.5 py-1.5 hover:bg-slate-50 dark:text-sky-100 dark:border-sky-500/40"
                        onClick={() =>
                          void post({
                      ...
                      <button
                        ...
                        onClick={() => {
                          if (
                            !confirm(
                              `Remove staff member ${p.name}? This does not delete appointment history.`
                            )
                          ) {
                            return;
                          }
                  ...
                  <div className="mt-3">
                      ...
                      <div className="mb-3">
                        <AdvisorIdentityPanel
                          ...
                          saving={saving}
                          inputClass={fc()}
                          toneClass="border-sky-200 bg-sky-50/70 dark:border-sky-700/50 dark:bg-sky-950/30"
                          personLabel={p.name || 'This team member'}
                        />
                      </div>

                      {isEditing && (
                        <div className="mt-3 rounded-xl border border-sky-300 bg-white p-3 space-y-2 dark:border-sky-600 dark:bg-sky-950/50">
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                            ...
                            <div className="flex flex-col justify-end gap-1.5 pb-0.5 sm:col-span-2">
                              <label className="inline-flex items-center gap-2 text-[11px] font-bold text-slate-700 dark:text-sky-100">
                                <input
                                  type="checkbox"
                          ...
                          <div>
                            ...
                            <div className="flex flex-wrap gap-1.5">
                              {skillOptions.map((s) => {
                                const on = profile.roles.includes(s);
                                return (
                                  <button
                                    key={s}
                                    type="button"
                                    onClick={() =>
                                      setProfile(p.id, {
                                        roles: toggleInList(profile.roles, s),
                                      })
                                    }
                                    className={`rounded-full border px-2.5 py-1 text-[11px] font-bold transition-colors ${
                                      on
                          ...
                          <div className="flex flex-wrap gap-1.5 pt-1">
                            <button
                              ...
                              className="rounded-xl bg-sky-600 px-3 py-1.5 text-[11px] font-bold text-white hover:bg-sky-700 disabled:opacity-50"
                            >
                              Save staff details
                            </button>
                            <button
                              ...
                              className="rounded-xl border border-slate-200 bg-white px-3 py-1.5 text-[11px] font-bold text-slate-700 hover:bg-slate-50 dark:border-sky-600/40 dark:bg-sky-950 dark:text-sky-100"
                            >
                              Cancel
                            </button>
                          </div>
                      ...
                      <div className="mt-3 rounded-xl border border-sky-200/80 bg-sky-50/60 p-2.5 dark:border-sky-700/50 dark:bg-sky-950/40">
                        <p className="text-[10px] font-black uppercase tracking-wider text-sky-800 dark:text-sky-300 mb-1.5">
                        ...
                        <div className="mt-2 flex flex-wrap gap-1.5">
                          <button
                            ...
                            className="rounded-xl bg-sky-600 px-2.5 py-1.5 text-[11px] font-bold text-white hover:bg-sky-700 disabled:opacity-50"
                          >
                            Save dates & rate
                          </button>
                          {isActive ? (
                          ...
                          {hist.length > 0 || p.end_date ? (
                            <button
                              ...
                              className="rounded-xl border border-slate-200 bg-white px-2.5 py-1.5 text-[11px] font-bold text-slate-700 hover:bg-slate-50 dark:border-sky-600/40 dark:bg-sky-950 dark:text-sky-100"
                            >
                        ...
                        {showHist && (
                          <ul className="mt-2 space-y-1 border-t border-sky-200/60 pt-2 dark:border-sky-700/40">
                            {p.end_date ? (
                            ...
                            {hist.length === 0 ? (
                              <li className="text-[11px] text-slate-500 dark:text-sky-200/60">
                              ...
                              hist.map((h) => (
                                <li
                                  key={h.id}
                                  className="text-[11px] text-slate-600 dark:text-sky-100/90"
                                >
                      ...
                      <div className="mt-3">
                        <FitContractDocsPanel
                          ...
                          title="Staff contract PDFs"
                          description="Employment agreements, NDAs, or rate letters for this team member (owner only — not on the public website)."
                          defaultKind="staff_agreement"
                          disabled={saving}
                          toneClass="border-sky-200/80 bg-sky-50/50 dark:border-sky-700/50 dark:bg-sky-950/30"
                        />
                      </div>
