...
export default function WebsitePage() {
  ...
  useEffect(() => {
    ...
    setForm({
      ...
      embed_primary_color: s.embed_primary_color || '#6366f1',
      ...
      {loading || !store ? (
        ...
        <div className="space-y-6">
          <AdvisorPortalManager
            ...
            portalPath={
              token
                ? `/embed/advisor/psychiatrygraph/${encodeURIComponent(token)}`
                : ''
            }
          ...
          <AdvisorDeskInviteCard module="psychiatrygraph" />
          ...
          {token ? (
            <AdvisorEmbedSnippet
              embedPath={`/embed/advisor/psychiatrygraph/${encodeURIComponent(token)}`}
          ...
          <AdvisorRoomsCard
            ...
            manageHref="/dashboard/psychiatrygraph/rooms"
            hint="Quick list. Open Rooms (Floor) for equipment and which clinicians use each room."
            onSave={async (rooms) => {
          ...
          {token ? (
            <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-slate-50/80 dark:bg-slate-900/40 p-4 space-y-2">
              ...
              <p className="text-[11px] font-mono break-all text-slate-700 dark:text-slate-200">
                /embed/advisor/psychiatrygraph/{token}
              </p>
              <a
                href={`/embed/advisor/psychiatrygraph/${token}`}
                target="_blank"
                rel="noreferrer"
          ...
          {token ? (
            <p className="text-[11px] font-mono text-indigo-800 dark:text-indigo-200 break-all">
              Public token: {token}
            </p>
          ) : null}
