...
export default function PatientsPage() {
  ...
  const openEdit = (p: PsychiatryPatient) => {
    setForm({
      id: p.id,
      code: p.code || '',
      name: p.name || '',
  ...
  const patchPatient = async (
    ...
    patch: Partial<PsychiatryPatient> & Record<string, unknown>
  ) => {
  ...
  const issuePortal = async (patientId: string) => {
    try {
      ...
      if (tok && typeof window !== 'undefined') {
        const url = `${window.location.origin}/member/psychiatrygraph/${encodeURIComponent(tok)}`;
  ...
  const invitePatient = async (p: PsychiatryPatient) => {
    try {
      const data = await post({
      ...
      {loading || !store ? (
        ...
        <div className="space-y-6">
          ...
          <AdvisorExpandablePanel
            ...
            description={
              editing
                ? 'Update contact details. Mental-health issues live on the patient chart.'
                : 'Register a patient. Capture psychiatry issues on their chart after saving.'
            }
                ...
                cells: [
                  ...
                  (
                    <span key="e" className="flex flex-wrap gap-2">
                      <Link
                        href={`/dashboard/psychiatrygraph/patients/${p.id}`}
                      ...
                      {p.portal_token ? (
                        <button
                          type="button"
                          className="inline-flex items-center gap-1 text-[11px] font-bold text-indigo-800"
                      ...
                      <button
                        type="button"
                        className="inline-flex items-center gap-1 text-[11px] font-bold text-indigo-900"
