...
export default function PatientsPage() {
  ...
  const patchPatient = async (
    patient: VetPatient,
    patch: Partial<VetPatient> & Record<string, unknown>
  ) => {
  ...
  const issuePortal = async (patientId: string) => {
    try {
      ...
      if (tok && typeof window !== 'undefined') {
        const url = `${window.location.origin}/member/vetgraph/${encodeURIComponent(tok)}`;
        await navigator.clipboard.writeText(url);
        toast.success(
          'Client portal link copied — they can book open diary slots'
        );
      } else {
        toast.success('Client portal issued');
      }
    } catch {
      /* toast in post */
    }
  ...
  const invitePatient = async (p: VetPatient) => {
    try {
      const data = await post({
  ...
  return (
    <VetgraphWorkbench
      ...
      description="Patient book with clinical profile, medical chart, and client portal links so clients can see open diary vacancies and book appointments themselves."
    >
      {loading || !store ? (
        <LoadingBlock />
      ) : (
            ...
            store.patients.find((x) => x.id === form.id)?.family?.length ? (
              <div className="sm:col-span-2 lg:col-span-3 rounded-2xl border border-teal-200 dark:border-teal-800 bg-teal-50/50 dark:bg-teal-950/30 p-3">
                <p className="text-[10px] font-black uppercase tracking-wide text-teal-800 dark:text-teal-200 mb-1.5">
                  Family members (from client portal)
                </p>
                ...
                cells: [
                  ...
                  (
                    <span key="e" className="flex flex-wrap gap-2">
                      <Link
                        href={`/dashboard/vetgraph/patients/${p.id}`}
