...
export default function VetPatientRecordPage() {
  const { id } = useParams() as { id: string };
  const search = useSearchParams();
  const { companyId, store, loading, saving, post, load } = useVetgraph();
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <PatientVisitHistory
              ...
              calendarHref={(v) =>
                `/dashboard/vetgraph/calendar?appointment=${encodeURIComponent(v.appointment_id)}`
              }
            />
          </div>
          ...
          <PatientMedicalChart
            ...
            claimPackHref={(claimId) =>
              `/api/clinic/medical-aid-claims/pack?companyId=${companyId}&module=vetgraph&patientId=${encodeURIComponent(patient.id)}&claimId=${encodeURIComponent(claimId)}`
            }
          />
        </div>
      )}
