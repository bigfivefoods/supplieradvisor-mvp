...
import {
  ...
  StatRow,
  fc,
} from '@/components/clinic/VetForm';
import {
...
function formatEngagement(p: VetPractitioner) {
  const start =
...
async function copyClinicianPortal(tok: string) {
  ...
  const url = `${window.location.origin}/clinician/vetgraph/${encodeURIComponent(tok)}`;
...
async function copyStaffToday(tok: string) {
  if (typeof window === 'undefined') return;
  const url = `${window.location.origin}/staff/advisor/vetgraph/${encodeURIComponent(tok)}`;
  await navigator.clipboard.writeText(url);
...
function profileFromPerson(p: VetPractitioner): ProfileDraft {
  return {
    code: p.code || '',
    name: p.name || '',
    email: p.email || '',
...
export default function PractitionersPage() {
  const { companyId, store, loading, saving, post, summary, load } = useVetgraph();
  ...
  const profileFor = (p: VetPractitioner): ProfileDraft =>
  ...
  const startEdit = (p: VetPractitioner) => {
  ...
  const draftFor = (p: VetPractitioner): DateDraft => {
  ...
  const saveDates = async (p: VetPractitioner) => {
    const d = draftFor(p);
    if (!d.start_date) {
  ...
  const endTenure = async (p: VetPractitioner) => {
    const d = draftFor(p);
  ...
  const rehire = async (p: VetPractitioner) => {
    const d = draftFor(p);
    const start =
  ...
  const saveIdentity = async (p: VetPractitioner) => {
  ...
  const saveContracts = async (
    ...
    await post({
      ...
      record: {
        ...
        code: p.code,
        name: p.name,
        contracts: contracts as VetContractDoc[],
      },
    });
  };

  const saveDetails = async (p: VetPractitioner) => {
    const pr = profileFor(p);
    if (!pr.name.trim()) {
      toast.error('Name required');
      return;
