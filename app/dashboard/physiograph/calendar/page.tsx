          ...
          kind === 'personal'
            ...
            : patients
              ...
              : a.public
                ? 'Open public slot'
                : undefined,
        tone: kind === 'personal' ? ('indigo' as const) : ('teal' as const),
      };
    });
  }, [store]);
      ...
      {loading || !store ? (
        ...
        <div className="space-y-6">
          ...
          <WorkingHoursEditor
            ...
            description="Open days and times for this physio clinic. Closed days are dimmed; day view uses your open window."
          ...
          <div className="space-y-2">
            <div className="flex flex-wrap items-center justify-between gap-2">
              ...
              {selectedId ? (
                <div className="flex flex-wrap gap-2">
                  <button
                    type="button"
                    className="rounded-xl border border-teal-300 bg-white px-3 py-1.5 text-xs font-bold text-teal-800"
                    onClick={() => startCreate({ date: form.date })}
                  >
                    + New appointment
                  </button>
            ...
            <FormCard
              title={
                selectedId
                  ? `Open ${form.appointment_kind === 'personal' ? 'block' : 'appointment'} · ${form.date} ${form.start_time}`
                  : form.appointment_kind === 'personal'
                    ? 'Block own time / leave'
                    : 'Schedule appointment'
              }
              onSubmit={() => void save()}
              saving={saving}
            ...
            >
              ...
              {selectedId && rosterOnSelected.length > 0 ? (
                <p className="sm:col-span-2 lg:col-span-3 text-[11px] text-slate-600 rounded-xl border border-teal-100 bg-teal-50/50 px-3 py-2">
              ...
              .map((a) => {
                ...
                return {
                  ...
                  cells: [
                    a.date,
                    a.end_time
                      ? `${a.start_time}–${String(a.end_time).slice(0, 5)}`
                      : a.start_time,
                    kind === 'personal'
                      ? appointmentKindLabel(kind, a.personal_reason)
                      : svc?.name || '—',
                    prac?.name || '—',
                    a.status,
                    a.public ? 'Yes' : 'No',
