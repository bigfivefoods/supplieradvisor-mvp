...
export default function BookingsPage() {
  ...
  const mark = async (id: string, status: string) => {
    ...
    if (status === 'attended') {
      ...
      if (tok) {
        const path = buildPublicFeedbackPath('physiograph', companyId, tok);
        const url =
  ...
  const copyFeedback = async (token: string) => {
    const path = buildPublicFeedbackPath('physiograph', companyId, token);
                ...
                cells: [
                  ...
                  b.feedback_submitted_at ? (
                    ...
                    <button
                      key="fb"
                      type="button"
                      className="inline-flex items-center gap-1 text-[10px] font-bold text-teal-700 dark:text-teal-300"
                  ...
                  (
                    <span key="a" className="flex flex-wrap gap-1">
                      ...
                      {pat ? (
                        <Link
                          href={`/dashboard/physiograph/patients/${pat.id}?appointment=${b.appointment_id}&booking=${b.id}${
                            apt?.practitioner_id
