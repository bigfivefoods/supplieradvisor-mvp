...
function formatEngagement(p: PhysioPractitioner) {
  const start =
...
async function copyClinicianPortal(tok: string) {
  ...
  const url = `${window.location.origin}/clinician/physiograph/${encodeURIComponent(tok)}`;
...
async function copyStaffToday(tok: string) {
  if (typeof window === 'undefined') return;
  const url = `${window.location.origin}/staff/advisor/physiograph/${encodeURIComponent(tok)}`;
  await navigator.clipboard.writeText(url);
...
function profileFromPerson(p: PhysioPractitioner): ProfileDraft {
  return {
    code: p.code || '',
    name: p.name || '',
    email: p.email || '',
...
export default function PractitionersPage() {
  const { companyId, store, loading, saving, post, summary, load } = usePhysiograph();
  ...
  const profileFor = (p: PhysioPractitioner): ProfileDraft =>
  ...
  const startEdit = (p: PhysioPractitioner) => {
  ...
  const draftFor = (p: PhysioPractitioner): DateDraft => {
  ...
  const saveDates = async (p: PhysioPractitioner) => {
    const d = draftFor(p);
    if (!d.start_date) {
  ...
  const endTenure = async (p: PhysioPractitioner) => {
    const d = draftFor(p);
  ...
  const rehire = async (p: PhysioPractitioner) => {
    const d = draftFor(p);
    const start =
  ...
  const saveIdentity = async (p: PhysioPractitioner) => {
  ...
  const saveContracts = async (
    ...
    await post({
      ...
      record: {
        ...
        code: p.code,
        name: p.name,
        contracts: contracts as PhysioContractDoc[],
      },
    });
  };

  const saveDetails = async (p: PhysioPractitioner) => {
    const pr = profileFor(p);
    if (!pr.name.trim()) {
      toast.error('Name required');
      return;
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-2">
                  ...
                  actions={
                    ...
                    <>
                      <button
                        type="button"
                        disabled={saving}
                        className="inline-flex items-center gap-1 text-xs font-bold text-teal-900 border border-teal-300 rounded-xl px-2.5 py-1.5 bg-teal-50 hover:bg-teal-100 dark:text-teal-100 dark:border-teal-500/50 dark:bg-teal-950 dark:hover:bg-teal-900/50"
                        onClick={() =>
                      ...
                      <button
                        type="button"
                        disabled={saving}
                        className="inline-flex items-center gap-1 text-xs font-bold text-slate-700 border border-slate-200 rounded-xl px-2.5 py-1.5 hover:bg-slate-50 dark:text-teal-100 dark:border-teal-500/40"
                        onClick={() =>
                          void post({
                ...
                >
                  ...
                  >
                    {p.photo_url ? (
                      ...
                      <img
                        src={p.photo_url}
                        alt=""
                        className="w-9 h-9 rounded-full object-cover border border-teal-200 dark:border-teal-600 shrink-0"
                      />
                    ) : (
                      <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-teal-200 bg-teal-50 text-[11px] font-black text-teal-800 dark:border-teal-600 dark:bg-teal-950 dark:text-teal-100">
                        {(p.name || 'P').slice(0, 1).toUpperCase()}
                      </span>
                    )}
                    <div className="min-w-0 flex-1">
                      <div className="font-bold text-sm text-slate-900 dark:text-teal-50 flex flex-wrap items-center gap-2">
                        <span>
                          {p.code} · {p.name}
                        </span>
                        <span
                      ...
                      <div className="text-[11px] text-slate-500 dark:text-teal-200/80">
                        {(p.disciplines || []).join(', ') || '—'}
                      ...
                      <div className="text-[11px] font-semibold text-teal-900/90 dark:text-teal-200 mt-0.5">
                        {formatEngagement(p)}
                        {' · '}
                    ...
                    <ChevronDown
                      className={`mt-1 h-4 w-4 shrink-0 text-teal-800 transition-transform dark:text-teal-200 ${
                        isOpen ? 'rotate-180' : ''
                      }`}
                    />
                  </button>
                  ...
                  <div className="mt-3">
                      {p.public_bio && !isEditing && (
                        <p className="text-[11px] text-slate-600 dark:text-teal-100/80 mb-2">
                          {p.public_bio}
                        </p>
                      )}
                      ...
                      <div className="mb-3">
                        <AdvisorIdentityPanel
                          ...
                          saving={saving}
                          inputClass={fc()}
                          toneClass="border-teal-200 bg-teal-50/70 dark:border-teal-700/50 dark:bg-teal-950/30"
                          personLabel={p.name || 'This practitioner'}
                        />
                      </div>

                      {isEditing && (
                        <div className="mt-3 rounded-xl border border-teal-300 bg-white p-3 space-y-2 dark:border-teal-600 dark:bg-teal-950/50">
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                            <label className="block">
                              <span className="text-[10px] text-slate-600 dark:text-teal-200/70">
                                Code
                              </span>
                              <input
                                className={fc() + ' mt-0.5'}
                            ...
                            <div className="flex flex-col justify-end gap-1.5 pb-0.5 sm:col-span-2">
                              <label className="inline-flex items-center gap-2 text-[11px] font-bold text-slate-700 dark:text-teal-100">
                                <input
                                  type="checkbox"
                          ...
                          <div>
                            <p className="text-[10px] font-black uppercase tracking-wider text-teal-800 dark:text-teal-300 mb-1.5">
                              Skills / disciplines
                            </p>
                          ...
                          <div className="flex flex-wrap gap-1.5 pt-1">
                            <button
                              ...
                              className="rounded-xl bg-teal-600 px-3 py-1.5 text-[11px] font-bold text-white hover:bg-teal-700 disabled:opacity-50"
                            >
                            ...
                            <button
                              ...
                              className="rounded-xl border border-slate-200 bg-white px-3 py-1.5 text-[11px] font-bold text-slate-700 hover:bg-slate-50 dark:border-teal-600/40 dark:bg-teal-950 dark:text-teal-100"
                            >
                              Cancel
                            </button>
                          </div>
                      ...
                      <div className="mt-3 rounded-xl border border-teal-200/80 bg-teal-50/60 p-2.5 dark:border-teal-700/50 dark:bg-teal-950/40">
                        <p className="text-[10px] font-black uppercase tracking-wider text-teal-800 dark:text-teal-300 mb-1.5">
                        ...
                        {resolveAdvisorEngagement(profile) === 'contractor' ? (
                          <div className="mt-2">
                            <AdvisorContractorCommercialFields
                              ...
                              inputClass={fc()}
                              labelClass="text-[10px] font-black uppercase tracking-wider text-teal-800 dark:text-teal-300"
                              hintClass="text-[11px] text-slate-500 dark:text-teal-200/70"
                        ...
                        <div className="mt-2 flex flex-wrap gap-1.5">
                          <button
                            ...
                            className="rounded-xl bg-teal-600 px-2.5 py-1.5 text-[11px] font-bold text-white hover:bg-teal-700 disabled:opacity-50"
                          >
                            Save dates & rate
                          </button>
                          {isActive ? (
                          ...
                          {hist.length > 0 || p.end_date ? (
                            <button
                              ...
                              className="rounded-xl border border-slate-200 bg-white px-2.5 py-1.5 text-[11px] font-bold text-slate-700 hover:bg-slate-50 dark:border-teal-600/40 dark:bg-teal-950 dark:text-teal-100"
                            >
                        ...
                        {showHist && (
                          <ul className="mt-2 space-y-1 border-t border-teal-200/60 pt-2 dark:border-teal-700/40">
                            {p.end_date ? (
                              <li className="text-[11px] text-slate-700 dark:text-teal-100">
                            ...
                            {hist.length === 0 ? (
                              <li className="text-[11px] text-slate-500 dark:text-teal-200/60">
                              ...
                              hist.map((h) => (
                                <li
                                  key={h.id}
                                  className="text-[11px] text-slate-600 dark:text-teal-100/90"
                                >
                      ...
                      <div className="mt-3">
                        <PersonQualificationsEditor
                          ...
                          disabled={saving}
                          toneClass="border-teal-200/80 bg-teal-50/50 dark:border-teal-700/50 dark:bg-teal-950/30"
                        />
                      </div>
            ...
            {store.practitioners.length === 0 ? (
              <p className="text-sm text-slate-500 dark:text-teal-200/70 py-6 text-center">
          ...
          <AdvisorExpandablePanel
            ...
            accentClass="border-teal-200 bg-teal-50/70 dark:border-teal-700/50 dark:bg-teal-950/40"
            titleClass="text-teal-950 dark:text-teal-50"
            hintClass="text-teal-900/80 dark:text-teal-200/80"
          >
          {/* Owner-managed skills / disciplines catalogue */}
          <div className="rounded-2xl border border-teal-200 bg-teal-50/70 p-4 space-y-3 dark:border-teal-700/50 dark:bg-teal-950/40">
            <div>
              <h3 className="text-sm font-black text-teal-950 dark:text-teal-50">
                Skills & disciplines
              </h3>
              <p className="text-[11px] text-teal-900/80 dark:text-teal-200/80 mt-0.5">
            ...
            <div className="flex flex-wrap gap-1.5">
              {skillOptions.map((s) => (
                <div
                  key={s}
                  className="inline-flex items-center gap-1 rounded-full border border-teal-300 bg-white pl-2.5 pr-1 py-0.5 text-[11px] font-bold text-teal-950 dark:border-teal-600 dark:bg-teal-950 dark:text-teal-100"
                >
                  {editSkillFrom === s ? (
                    <input
                      className="w-28 rounded-md border border-teal-400 bg-white px-1.5 py-0.5 text-[11px] font-bold dark:bg-teal-900"
                      value={editSkillTo}
                      autoFocus
                  ...
                  {editSkillFrom === s ? (
                    <>
                      <button
                        ...
                        className="rounded-full bg-teal-600 px-2 py-0.5 text-[10px] text-white"
                      >
                        Save
                      </button>
                      <button
                        ...
                        className="rounded-full px-1.5 py-0.5 text-[10px] text-slate-600 dark:text-teal-200"
                      >
                        Cancel
                      </button>
                    </>
                    ...
                    <>
                      <button
                        ...
                        className="rounded-full p-1 hover:bg-teal-100 dark:hover:bg-teal-900"
                      >
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <button
                ...
                className="inline-flex items-center gap-1 rounded-xl bg-teal-600 px-3 py-2 text-xs font-bold text-white hover:bg-teal-700 disabled:opacity-50"
              >
          ...
          >
            ...
            <label className="block">
              ...
              />
              <span className="mt-0.5 block text-[10px] text-slate-500 dark:text-teal-200/70">
