...
export default function BookingsPage() {
  ...
  const mark = async (id: string, status: string) => {
    ...
    if (status === 'attended') {
      ...
      if (tok) {
        const path = buildPublicFeedbackPath('medicalgraph', companyId, tok);
        const url =
  ...
  const copyFeedback = async (token: string) => {
    const path = buildPublicFeedbackPath('medicalgraph', companyId, token);
                ...
                cells: [
                  ...
                  (
                    <span key="a" className="flex flex-wrap gap-1">
                      ...
                      {pat ? (
                        <Link
                          href={`/dashboard/medicalgraph/patients/${pat.id}?appointment=${b.appointment_id}&booking=${b.id}${
                            apt?.practitioner_id
