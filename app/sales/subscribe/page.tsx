...
export default function SalesSubscribePage() {
  ...
  if (subscription?.isActive) {
    return (
      <div className="max-w-lg mx-auto rounded-3xl border border-emerald-400/30 bg-emerald-500/10 p-8 text-center">
        <CheckCircle2 className="w-12 h-12 text-emerald-400 mx-auto mb-4" />
        <h1 className="text-2xl font-black text-white">Subscription active</h1>
        ...
        <button
          ...
          className="mt-6 px-6 py-3 rounded-2xl bg-white text-slate-900 font-bold text-sm"
        >
          Open command centre
        </button>
      </div>
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      <div className="text-center">
        ...
        <p className="mt-2 text-slate-400 text-sm sm:text-base max-w-md mx-auto">
          Sell for <strong className="text-slate-200">{companyName || 'your company'}</strong> with
          full access — commission tools, pipeline, and forecasts.
        </p>
      </div>

      <div className="rounded-[2rem] border border-white/10 bg-gradient-to-br from-amber-500/20 via-orange-600/10 to-slate-900 p-6 sm:p-8 relative overflow-hidden">
        <Sparkles className="absolute top-4 right-4 w-8 h-8 text-amber-300/40" />
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            <div className="text-sm text-amber-200/90 font-semibold">Monthly rate</div>
          ...
          <div className="text-right">
            <div className="inline-flex items-center gap-1.5 text-xs font-bold uppercase tracking-wide text-orange-200 bg-orange-500/20 border border-orange-400/30 rounded-full px-3 py-1">
        ...
        <ul className="mt-6 space-y-2 text-sm text-slate-200">
          ...
          <li className="flex gap-2">
            <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0 mt-0.5" />
            Live commission preview — rates from <strong>3.5%</strong> up to <strong>5.5%</strong>
          </li>
          <li className="flex gap-2">
            <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0 mt-0.5" />
        ...
        {!agreementSigned && (
          <p className="mt-4 text-sm text-amber-200 bg-amber-500/10 border border-amber-400/30 rounded-2xl px-4 py-3">
        ...
        <button
          ...
          className="mt-6 w-full inline-flex items-center justify-center gap-2 px-6 py-4 rounded-2xl bg-gradient-to-r from-amber-500 to-orange-500 text-white font-black text-base shadow-xl shadow-orange-500/30 disabled:opacity-50"
        >
          {paying ? (
