...
/**
 * PsychiatryAdvisor® patient portal — registered patients book open diary slots.
 */
...
export default function MemberPsychiatrygraphPortalPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/public/psychiatrygraph/patient?token=${encodeURIComponent(token)}`,
        { cache: 'no-store' }
      );
  ...
  const color = portal?.primary_color || '#6366f1';
      ...
      >
        <div className="max-w-lg mx-auto">
          <p className="text-[10px] font-bold uppercase tracking-widest text-white/80">
            Patient portal · PsychiatryAdvisor®
          </p>
        ...
        {tab === 'care' && (
          <div className="rounded-2xl border border-rose-200 bg-white p-4 space-y-3">
            <div className="flex items-center gap-2 text-rose-800">
              ...
              <h2 className="text-sm font-black">Your care information</h2>
            </div>
            ...
            Object.keys(portal.medical_share).length > 0 ? (
              ...
              <p className="text-sm text-slate-500">
                Your practice has not shared a care summary yet.
              </p>
            )}
            <p className="text-[11px] text-slate-400">
              Summary only — clinical notes stay with your team.
            </p>
          </div>
        )}
                ...
                >
                  <div>
                    ...
                    <a
                      ...
                      href={`/api/public/advisor/ics?module=psychiatrygraph&date=${encodeURIComponent(b.date)}&start=${encodeURIComponent(b.start_time)}&title=${encodeURIComponent(b.service_name || 'Appointment')}&duration=45`}
                    >
                      Add to calendar
                    </a>
