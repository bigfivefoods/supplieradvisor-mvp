...
/**
 * PhysioAdvisor® patient portal — registered patients book open diary slots.
 */
...
export default function MemberPhysiographPortalPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/public/physiograph/patient?token=${encodeURIComponent(token)}`,
        { cache: 'no-store' }
      );
  ...
  const color = portal?.primary_color || '#0d9488';
          ...
          <div className="rounded-2xl border border-teal-200 bg-white p-4 space-y-3">
            ...
            Object.keys(portal.medical_share).length > 0 ? (
              ...
              <p className="text-sm text-slate-500">
                Your clinic has not shared clinical details yet. Contact the
                practice if you expected something here.
              </p>
            )}
                  ...
                  <div className="mt-3 flex flex-wrap gap-2 items-center">
                    ...
                    {s.my_status ? (
                      ...
                      <button
                        ...
                        className="rounded-xl bg-teal-600 px-3 py-1.5 text-xs font-bold text-white disabled:opacity-50"
                      >
                        {busyId === s.id
                          ? '…'
                          : s.full
                ...
                >
                  <div>
                    ...
                    <a
                      ...
                      href={`/api/public/advisor/ics?module=physiograph&date=${encodeURIComponent(b.date)}&start=${encodeURIComponent(b.start_time)}&title=${encodeURIComponent(b.service_name || 'Appointment')}&duration=45`}
                    >
                      Add to calendar
                    </a>
