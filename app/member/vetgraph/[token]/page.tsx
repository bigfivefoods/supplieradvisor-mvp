...
/**
 * VetAdvisor® client portal — registered patients book open diary slots.
 */
...
export default function MemberVetgraphPortalPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/public/vetgraph/patient?token=${encodeURIComponent(token)}`,
        { cache: 'no-store' }
      );
  ...
  const post = async (body: Record<string, unknown>) => {
    const res = await fetch('/api/public/vetgraph/patient', {
      method: 'POST',
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-6">
        <div className="max-w-md rounded-2xl border border-rose-200 bg-white p-6 text-center">
          <p className="font-black">Client portal unavailable</p>
  ...
  const copy = clinicPwaCopy('vetgraph');
  ...
  return (
    ...
    >
        ...
        {tab === 'history' && (
          <div className="space-y-3">
            ...
            <PatientVisitHistory
              ...
              rateHref={(v) =>
                publicRatePath('vetgraph', companyId, v.feedback_token)
              }
            />
        ...
        <p className="text-center text-[10px] text-slate-400 pb-8">
          Powered by VetAdvisor® · SupplierAdvisor
        </p>
    </MemberAdvisorShell>
    </>
  );
