...
export default function Onboarding() {
  ...
  const saveAll = async () => {
    ...
    try {
      ...
      toast.success("🎉 Business verified and saved to Supabase!");
  ...
  const steps = [
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <label htmlFor="logo-upload" className="btn-primary cursor-pointer w-full">Upload Logo</label>
          </div>
        </div>
      </div>
    ),
    ...
    () => (
      <div>
        ...
        <div className="bg-slate-100 p-8 rounded-3xl text-sm overflow-auto max-h-96">
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] pl-0 pr-12 py-12">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row gap-4 mt-10">
          {step > 0 && <button onClick={() => setStep(s => s - 1)} className="flex-1 border px-8 py-4 rounded-3xl font-medium">← Back</button>}
          <button onClick={() => setStep(s => s + 1)} className="flex-1 border px-8 py-4 rounded-3xl font-medium">Skip this Section</button>
          <button onClick={handleNext} disabled={loading} className="flex-1 bg-[#00b4d8] text-white py-4 rounded-3xl font-semibold flex items-center justify-center gap-3 disabled:opacity-70">
            {loading ? 'Saving to Supabase...' : (step === 6 ? 'Submit & Go Live' : 'Continue')} <ArrowRight />
          </button>
        </div>
      </div>
    </div>
