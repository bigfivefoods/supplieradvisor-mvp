 ...
 * List customer RIADs with container-style status buckets + summary.
 */
export async function GET(request: NextRequest) {
  try {
    ...
    if (customerId && Number.isFinite(customerId)) q = q.eq('customer_id', customerId);
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (error && /column|schema cache/i.test(error.message || '')) {
      ...
      const retry = await supabase.from('customer_riad').insert(minimal).select('*').single();
      data = retry.data;
      error = retry.error;
    }
