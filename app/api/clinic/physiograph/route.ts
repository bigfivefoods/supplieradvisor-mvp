...
export async function POST(request: NextRequest) {
  try {
    ...
    if (
      ...
      void import('@/lib/b2c/directory').then(({ indexBrandPerson }) =>
        indexBrandPerson({
          ...
          phone: patient.phone,
          portalToken,
          portalPath: `/member/physiograph/${encodeURIComponent(portalToken)}`,
        })
      );
      return NextResponse.json({
        success: true,
    ...
    if (
      ...
      const created: PhysioAppointment[] = planned.rows.map((r) => ({
        id: r.id,
        ...
        personal_reason: (r.personal_reason as PhysioAppointment['personal_reason']) ?? fields.personal_reason ?? null,
      ...
      const famId = body.family_member_id
        ...
        : null;
      const bookingsCreated: PhysioBooking[] = [];
      if (
        kind !== 'personal' &&
        patientId &&
      ...
      if (entity === 'practitioners') {
        ...
        if (person) {
          ...
          peopleSync = await syncStoreStaffPersonToHr({
            companyId,
            source: 'physiograph_practitioner',
            person: {
              ...person,
              employment_type:
...
function upsert(
  ...
  if (entity === 'practitioners') {
    ...
    store.services = ensureSystemPersonalService(store.services);
    const row: PhysioAppointment = applyAppointmentKindRules(
      {
      id,
