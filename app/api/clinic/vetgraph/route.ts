...
import {
  ...
  type VetPackage,
  type VetPatient,
  type VetPractitioner,
  type VetPublicSettings,
  type VetService,
  type VetgraphStore,
...
function analysis(store: VetgraphStore) {
...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      success: true,
      store,
      summary: summariseVetgraph(store),
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === 'seed_demo') {
      const demo = seedDemoVetgraph(now, companyId);
      await saveStore(companyId, meta, demo);
      return NextResponse.json({
        success: true,
        store: demo,
        summary: summariseVetgraph(demo),
        ...
        try {
          ...
          try {
            ...
            const mail = await notifyMembersOnServiceThread({
              ...
              portalBasePath: '/member/vetgraph',
            });
            if (mail.emailed > 0) {
              fanOut = {
            ...
            console.warn('[vetgraph] patient email notify failed', mailErr);
          }

        } catch (e) {
          console.warn('[vetgraph] service→company fan-out failed', e);
        }
      }
    ...
    if (action === 'update_settings') {
      const patch = (body.settings || body.record || {}) as Partial<VetPublicSettings>;
      ...
      if (body.rotate_token === true) {
        store.settings.public_token = `vetg_${companyId}_${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 10)}`;
      }
      await saveStore(companyId, meta, store);
      return NextResponse.json({
        success: true,
    ...
    /** Owner: issue client portal so they can book open diary slots */
    if (
      ...
      void import('@/lib/b2c/directory').then(({ indexBrandPerson }) =>
        indexBrandPerson({
          ...
          phone: patient.phone,
          portalToken,
          portalPath: `/member/vetgraph/${encodeURIComponent(portalToken)}`,
        })
      );
      return NextResponse.json({
        success: true,
        ...
        analysis: analysis(store),
        message: 'Client portal link issued',
      });
    }
    ...
    if (
      ...
      const created: VetAppointment[] = planned.rows.map((r) => ({
        id: r.id,
      ...
      const famId = body.family_member_id
        ...
        : null;
      const bookingsCreated: VetBooking[] = [];
      if (
        kind !== 'personal' &&
        patientId &&
  ...
  if (entity === 'practitioners') {
    ...
    let history = Array.isArray(prev?.history)
      ...
      : Array.isArray(rec.history)
        ? (rec.history as VetPractitioner['history'])
        : [];
    ...
    const row: VetPractitioner = {
      ...
      contracts: Array.isArray(rec.contracts)
        ? (rec.contracts as VetPractitioner['contracts'])
        : prev?.contracts || [],
      history,
      portal_token:
    ...
    store.services = ensureSystemPersonalService(store.services);
    const row: VetAppointment = applyAppointmentKindRules(
      {
      id,
      ...
      status: (rec.status as VetAppointment['status']) || prev?.status || 'scheduled',
    ...
    const nextStatus =
      (rec.status as VetBooking['status']) || prev?.status || 'booked';
    let row: VetBooking = {
      id,
