/**
 * VetAdvisor client portal (token auth).
...
async function saveStore(
  ...
  const { VETGRAPH_META_KEY } = await import('@/lib/clinic/vetgraph');
  ...
  try {
    ...
    if (action === 'update_profile') {
      ...
      return NextResponse.json({
        ...
        message: portalProfileSaveMessage(result, body, 'clinic records'),
      });
    }
    ...
    if (action === 'book' || action === 'request_join' || action === 'join') {
      ...
      if (bookedSlot.status === 'waitlist' && appt) {
        ...
        await notifyPracticeWaitlist({
          ...
          deskUrl: '/dashboard/vetgraph/bookings',
        });
      }
      ...
      await notifyPatientBookingPush({
        ...
        portalPath: patient.portal_token
          ? `/member/vetgraph/${patient.portal_token}?tab=mine`
          : '/me',
      });
      return NextResponse.json({
        success: true,
    ...
    console.error('[vetgraph client portal]', e);
    return NextResponse.json(
      { error: e instanceof Error ? e.message : 'Error' },
      { status: 500 }
    );
