/**
 * PsychiatryAdvisor patient portal (token auth).
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === 'book' || action === 'request_join' || action === 'join') {
      ...
      if (bookedSlot.status === 'waitlist' && appt) {
        ...
        await notifyPracticeWaitlist({
          ...
          deskUrl: '/dashboard/psychiatrygraph/bookings',
        });
      }
      ...
      await notifyPatientBookingPush({
        ...
        portalPath: patient.portal_token
          ? `/member/psychiatrygraph/${patient.portal_token}?tab=mine`
          : '/me',
      });
      return NextResponse.json({
        success: true,
    ...
    console.error('[psychiatrygraph patient portal]', e);
    return NextResponse.json(
      { error: e instanceof Error ? e.message : 'Error' },
      { status: 500 }
    );
