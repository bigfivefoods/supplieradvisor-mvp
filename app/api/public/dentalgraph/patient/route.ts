/**
 * DentalAdvisor patient portal (token auth).
...
async function resolvePatient(token: string): Promise<{
  ...
  const loaded = await loadAdvisorStoreForPublicToken({
    token: clean,
    moduleKey: DENTALGRAPH_META_KEY,
    read: readDentalgraphFromMetadata,
    parseCompanyId: parseDentalCompanyIdFromToken,
    indexKeys: [DENTALGRAPH_PATIENT_TOKENS_KEY],
  });
...
async function saveStore(
  ...
  const { DENTALGRAPH_META_KEY } = await import('@/lib/dental/dentalgraph');
...
export async function POST(request: NextRequest) {
  try {
    const ip = clientIp(request);
    const rl = rateLimit({
      key: `public-dental-patient-post:${ip}`,
      limit: 40,
      windowMs: 60 * 1000,
    });
    if (!rl.ok) {
    ...
    if (
      ...
      const clin = (store.staff || []).find((x) => x.id === cid);
      if (
        clin &&
    ...
    console.error('[dentalgraph patient portal]', e);
    return NextResponse.json(
      { error: e instanceof Error ? e.message : 'Error' },
      { status: 500 }
    );
