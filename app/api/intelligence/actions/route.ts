    ...
    if (action === 'task') {
      ...
      const { data: task, error } = await supabase
        ...
        .insert({
          ...
          title,
          description: detail || `From insight (${domain}). Open: ${href}`,
          status: 'todo',
          column_key: 'backlog',
          created_by: mem.userId,
          updated_at: now,
      ...
      if (error) {
        return NextResponse.json(
          { error: error.message, href: '/dashboard/projects/kanban-boards' },
          { status: 503 }
        );
      }
      return NextResponse.json({
        ...
        message: 'Task created on Intelligence actions project.',
      });
    }
    ...
    if (projectId) {
      ...
      const { data: riad, error } = await supabase
        ...
        .insert({
          ...
          status: 'open',
          severity,
          description: [detail, href ? `Source: ${href}` : ''].filter(Boolean).join('\n'),
          created_by: mem.userId,
          updated_at: now,
        })
        .select('id')
      ...
      if (!error && riad) {
        return NextResponse.json({
          ...
          message: 'RIAD item logged on project register.',
        });
      }
    }
